...
class EligibilityService {
    async runAssessment(applicationId) {
        ...
        const { data: employment } = await supabase_js_1.supabaseAdmin
            ...
            .maybeSingle();
        const { data: creditScore } = await supabase_js_1.supabaseAdmin
            .from('credit_scores')
            .select('*')
            .eq('profile_id', userId)
        ...
        if (!profile || !employment || !creditScore) {
            throw new app_error_js_1.BadRequestError('User profile, employment, and credit score are all required for eligibility assessment');
        }
        // 3. Assessment Logic
        const reasons = [];
        let isEligible = true;
        ...
        const { data: existingResult } = await supabase_js_1.supabaseAdmin
